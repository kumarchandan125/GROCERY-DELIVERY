...
const checkLowStock = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("send-low-stock-email", async () => {
      ...
      //Nodemailer to send email to admin
      await sendEmail({
        to: adminEmails.join(","),
  ...
  async ({ step }) => {
    const { deals, users } = await step.run(
      ...
      async () => {
        ...
        return { deals: product, users: allUsers };
      },
    );
...
//Auto Assign Rider after 5 minute
  ...
  async ({ event, step }) => {
    ...
    const result = await step.run("assign-rider", async () => {
      ...
      if (!order) {
        return {
          skipped: true,
          reason: "order not found or already cancelled",
        };
      }
      ...
      if (order.deliveryPartnerId) {
        return {
          skipped: true,
          reason: "Already assigned to a rider",
        };
      }
      ...
      //Find an active rider not currently on another delivery
