          ...
          {steps.map((s, i) => (
            <div key={s.key} className="flex items-center gap-2">
              <button
                ...
                className={`flex item-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${step === s.key ? "bg-app-green text-white" : "bg-white text-app-text-light"}`}
              >
                <s.icon className="size-4" />
                {s.label}
                {i < step.length - 1 && <ChevronRightIcon className="size-4" />}
              </button>
            </div>
          ))}
        </div>
